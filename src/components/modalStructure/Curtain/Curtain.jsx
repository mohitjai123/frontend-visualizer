...
const Curtain = ({
  ...
  const curtainTemplate = item?.templates;
  const currentTextures =
    item?.uploadOptions[0]?.image ||
    curtainTemplate[modelIndex]?.defaultTexture;

    const modelConfigs = useMemo(() => {
      const configs = [
        {
          modelUrl: curtainTemplate[modelIndex]?.add_curtain,
          textureUrl: currentTextures,
          shadowTextureUrl: curtainTemplate[modelIndex]?.shadow_png,
        },
        {
          modelUrl: curtainTemplate[modelIndex]?.body_glb,
          textureUrl: curtainTemplate[modelIndex]?.body_png,
        },
      ];
    
      // Add background model configuration conditionally
      if (curtainTemplate[modelIndex]?.background_glb) {
        configs.push({
          modelUrl: curtainTemplate[modelIndex]?.background_glb,
          textureUrl: curtainTemplate[modelIndex]?.background_png,
        });
      }
    
      return configs;
    }, [modelIndex, currentTextures, curtainTemplate]);
